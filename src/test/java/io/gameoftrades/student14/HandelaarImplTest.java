...
public class HandelaarImplTest {
    ...
    @Test
    //Zelf bedachte test
    //Eiland kaart geladen er is geen optie om naar een andere stad te gaan
    public void zouGeenPadMoetenVinden() {
        ...
        //https://coderanch.com/t/96059/engineering/handle-NullPointerException-tests
    }

    @Test
    //Zelf bedachte test
    //Controleren of het echt de snelste pad pakt
    public void zouSnelstePadMoetenVinden()
    {
        Wereld wereld = handelaar.nieuweWereldLader().laad("/kaarten/testcases/vierkant-kaart.txt");
        assertNotNull(wereld);
        ...
        List<Stad> list = algoritme.bereken(kaart, wereld.getSteden());


        assertEquals("Brian", list.get(0).getNaam());
        assertEquals("Dominic", list.get(1).getNaam());
        assertEquals("Letty", list.get(2).getNaam());
        assertEquals("Mia", list.get(3).getNaam());
    }
